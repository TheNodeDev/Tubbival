...
auto Client::SetPortalGunIndicatorColor(Vector v) -> void {
...
auto Client::UpdatePortalGunIndicatorColor() -> void {
    ...
    while ( ( particleSystem = client->GetParticleSystem(particleSystem) ) ) {
        ...
        if ( controlPointCount == PORTAL_GUN_CUSTOM_INDICATOR_CP_ID + 2 && particleCollection.m_Center.Length() == 0 ) {
            ...
            if ( newColor.Length() == 0 )
				newColor = m_pCPInfo[1].m_ControlPoint.m_Position;
        ...
        particleCount++;
        if ( particleCount > 1024 )
			break; // in case something fucks up
    }
}

Client::Client() : Module() { }

auto Client::Init( PluginBase* plugin ) -> bool {
...
//    }
    ...
    if (! !leaderboard ) {
        ...
        using _FindElement = void*(__funccc*)(void* thisptr, const char* pName);
