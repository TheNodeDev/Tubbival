...
void CelesteMoveset::UpdateModeParams() {
    portal2Sdk.SetModeParam(DashesLeft, dashesLeft);
    portal2Sdk.SetModeParam(StaminaLeft, climbStamina);
    if (!portal2Sdk.GetModeParam(InitialValue)) {
        portal2Sdk.SetModeParam(MaxDashes, maxDashes);
        portal2Sdk.SetModeParam(MaxStamina, wallClimbMaxStamina);
        portal2Sdk.SetModeParam(InitialValue, 1);
    }
    maxDashes = portal2Sdk.GetModeParam(MaxDashes);
    wallClimbMaxStamina = portal2Sdk.GetModeParam(MaxStamina);
    portal2Sdk.SetModeParam(Dashing, (dashing > 0) ? 1 : 0);
}
...
void IN_DashDown(const CCommand& args) {
    if(portal2Sdk.GetMode() == Celeste)celesteMoveset.Dash();
}
